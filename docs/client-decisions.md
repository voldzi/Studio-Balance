...
| CD-010 | Produkční DB bootstrap dočasně použije přes HAProxy `sslmode=prefer`; Keycloak realm použije stávající hostname `login.zeleznalady.cz` | CA pro `verify-full` nebyla dohledána; tento režim je přechodný. Vyhrazený realm a přesné client callbacky jsou zachované |
