...
if set(values) != {'OIDC_OPERATIONS_CLIENT_ID','OIDC_OPERATIONS_CLIENT_SECRET'} or values['OIDC_OPERATIONS_CLIENT_ID'] != 'studio-balance-operations': raise SystemExit('Only dedicated registration credentials are accepted')
secret = values['OIDC_OPERATIONS_CLIENT_SECRET']
if not secret or any(c not in 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-.' for c in secret): raise SystemExit('Invalid credential format')
target = root / '.env.production'
lines = [line for line in target.read_text().splitlines() if line.split('=',1)[0] not in values]
fd, temporary = tempfile.mkstemp(prefix='.registration-env-',dir=root)
