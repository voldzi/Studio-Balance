...
import { LessonInstructors } from "./lesson-instructors";
...
export function LessonDetail({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    setState("loading"); setLesson(undefined);
    void apiRequest<ClassTypeDetail>(`/api/v1/class-types/${slug}`, { signal: controller.signal })
      .then((result) => { setLesson(result); setState("ready"); })
  ...
  return <article className="lesson-detail">
    ...
    <LessonInstructors instructors={lesson.instructors} />
