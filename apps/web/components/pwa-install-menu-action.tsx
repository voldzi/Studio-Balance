...
export function PwaInstallMenuAction() {
  ...
  return (
    <>
      <button aria-haspopup="dialog" className="mobile-nav-install" onClick={() => setOpen(true)} type="button">
        Přidat aplikaci
      </button>
      {open && (
        <div aria-labelledby="pwa-mobile-install-title" aria-modal="true" className="pwa-install-modal" role="dialog">
          <button aria-label="Zavřít návod k instalaci" className="pwa-install-modal-backdrop" onClick={() => setOpen(false)} type="button" />
