...
export function AdminDashboard({ email }: { email: string }) {
  ...
  async function saveClass(event: FormEvent<HTMLFormElement>) {
    ...
    await mutate(editingClass ? `/api/v1/admin/class-types/${editingClass.id}` : "/api/v1/admin/class-types", { method: editingClass ? "PATCH" : "POST", body: JSON.stringify(payload) }, editingClass ? "Typ lekce byl upraven." : "Typ lekce byl přidán.");
