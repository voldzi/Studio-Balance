...
import { StudioOperationsEditor } from "./studio-operations-editor";
import { InstructorEditor, TeamEditor, type AdminInstructor } from "./instructor-editor";
import { ScheduleBatchForm } from "./schedule-batch-form";
...
type Instructor = AdminInstructor;
type Session = { arrivalLeadMinutes: number; bookingCount: number; capacity: number; changeNotice: string | null; className: string; classTypeId: string; endAt: string; equipment: string; id: string; instructorId: string; instructorName: string; locationAddress: string; locationName: string; priceCents: number; startAt: string; status: string; suitability: string };
...
type Dashboard = {
  ...
  classPopularity: { attended: number; className: string; classTypeId: string; reservations: number }[];
  clients: number;
  metrics: {
  ...
  nextWeek: Session[];
  today: Session[];
  weeklyAttendance: { attended: number; weekStart: string }[];
};
...
export function AdminDashboard({ email }: { email: string }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setDashboard(nextDashboard); setClassTypes(nextClasses.items); setInstructors(nextInstructors.items); setSessions(nextSessions.items); setClients(nextClients.items); setBookings(nextBookings.items); setReviews(nextReviews.items); setTransformations(nextTransformations.items); setNews(nextNews.items); setSessionExpired(false);
    } catch (error) { if (error instanceof ApiError && error.status === 401) setSessionExpired(true); else setMessage(error instanceof Error ? error.message : "Administraci se nepodařilo načíst."); }
  }, []);
  ...
  async function mutate(path: string, init: RequestInit, success: string): Promise<boolean> {
    ...
    catch (error) { if (error instanceof ApiError && error.status === 401) setSessionExpired(true); else setMessage(error instanceof Error ? error.message : "Změnu se nepodařilo uložit."); return false; }
    finally { setBusy(false); }
  }

  async function saveSession(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = event.currentTarget; const data = new FormData(form);
    if (editingSession?.bookingCount && !window.confirm(`Tento termín má ${editingSession.bookingCount} aktivních rezervací. Klientkám vznikne zpráva o změně a odešle se provozní e-mail. Pokračovat?`)) return;
    const payload = { classTypeId: data.get("classTypeId"), instructorId: data.get("instructorId"), startAt: new Date(String(data.get("startAt"))).toISOString(), durationMinutes: Number(data.get("durationMinutes")), arrivalLeadMinutes: Number(data.get("arrivalLeadMinutes")), locationName: data.get("locationName"), locationAddress: data.get("locationAddress"), priceCents: Math.round(Number(data.get("price")) * 100), capacity: Number(data.get("capacity")), equipment: data.get("equipment"), suitability: data.get("suitability"), ...(editingSession ? { changeReason: data.get("changeReason") } : {}) };
    if (await mutate(editingSession ? `/api/v1/admin/sessions/${editingSession.id}` : "/api/v1/admin/sessions", { method: editingSession ? "PATCH" : "POST", body: JSON.stringify(payload) }, editingSession ? "Termín byl upraven a případné rezervované klientky byly informovány." : "Termín byl přidán.")) {
      setEditingSession(null); form.reset();
    }
  }
  ...
  async function toggleInstructor(item: Instructor) {
    const nextActive = false;
    if (!window.confirm(`Opravdu chcete instruktora ${item.displayName} odebrat z aktuální nabídky? Historické lekce zůstanou beze změny.`)) return;
    await mutate(`/api/v1/admin/instructors/${item.id}`, {
      method: "PATCH",
      body: JSON.stringify({ displayName: item.displayName, bio: item.bio, active: nextActive, sortOrder: item.sortOrder })
    }, "Instruktor byl odebraný z aktuální nabídky.");
  }
  ...
  async function changeScheduleBatch(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const data = new FormData(event.currentTarget);
    const action = String(data.get("action"));
    const classType = classTypes.find((item) => item.id === data.get("classTypeId"));
    const from = String(data.get("from")); const to = String(data.get("to")); const reason = String(data.get("reason"));
    if (!classType || !from || !to || !reason) return;
    const actionLabel = action === "cancel" ? "pozastaví a zruší" : "přesune";
    if (!window.confirm(`${actionLabel} všechny budoucí vyhovující termíny lekce ${classType.name} v zadaném období. Rezervované klientky dostanou provozní informaci. Pokračovat?`)) return;
    const payload = action === "cancel"
      ? { action, classTypeId: classType.id, from: new Date(`${from}T00:00:00`).toISOString(), to: new Date(`${to}T00:00:00`).toISOString(), reason }
      : { action, classTypeId: classType.id, from: new Date(`${from}T00:00:00`).toISOString(), to: new Date(`${to}T00:00:00`).toISOString(), reason, sourceWeekday: Number(data.get("sourceWeekday")), targetWeekday: Number(data.get("targetWeekday")), targetTime: data.get("targetTime") };
    if (await mutate("/api/v1/admin/sessions/batch", { method: "POST", body: JSON.stringify(payload) }, action === "cancel" ? "Vyhovující budoucí termíny byly pozastaveny a klientky s rezervací byly informovány." : "Vyhovující budoucí termíny byly přesunuty a klientky s rezervací byly informovány.")) event.currentTarget.reset();
  }
  ...
  return <main className="admin-app"><aside className="admin-sidebar"><div><p className="admin-brand">Studio Balance</p><p className="admin-caption">Administrace</p></div><nav aria-label="Administrace">{tabs.map((item) => <button aria-current={tab === item.id ? "page" : undefined} className={tab === item.id ? "active" : ""} key={item.id} onClick={() => setTab(item.id)} type="button">{item.label}</button>)}</nav><div className="admin-user"><span>{email}</span><form action="/admin/auth/logout" method="post"><button type="submit">Odhlásit se</button></form></div></aside><section className="admin-content">{sessionExpired && <div className="admin-message" role="alert"><strong>Přihlášení do administrace vypršelo.</strong> Rozepsané údaje zůstanou na této stránce. <a href="/admin/prihlaseni" rel="noopener noreferrer" target="_blank">Přihlaste se znovu v nové kartě</a>, potom se sem vraťte a uložení zopakujte. <button onClick={() => void load()} type="button">Ověřit přihlášení</button></div>}{message && <p className="admin-message" role="status">{message}</p>}{!dashboard ? <p role="status">Načítáme administraci…</p> : <>
    {tab === "dashboard" && <><StudioOperationsEditor /><AdminHeading eyebrow="Dnešní provoz" title="Přehled studia" copy="Aktuální provoz, návštěvnost a vývoj za poslední týdny." /><div className="admin-stats"><Stat label="Dnešní lekce" value={dashboard.today.length} /><Stat label="Aktivní rezervace" value={dashboard.activeBookings} /><Stat label="Klienti" value={dashboard.clients} /></div><div className="admin-stats admin-stats-secondary"><Stat help="Počet aktivních nebo již vyhodnocených rezervací na lekce v tomto kalendářním týdnu." label="Rezervace tento týden" value={dashboard.metrics.reservationsThisWeek} /><Stat help="Jen návštěvy, které byly v administraci označené jako účast." label="Návštěvy tento měsíc" value={dashboard.metrics.attendedThisMonth} /><Stat help="Podíl potvrzených účastí z účastí a neúčastí za posledních 90 dní. Nevyhodnocené rezervace se nezapočítávají." label="Docházka za 90 dní" value={dashboard.metrics.attendanceRate90Days === null ? "—" : `${dashboard.metrics.attendanceRate90Days} %`} /><Stat help="Pouze součet cen rezervací označených jako účast. Nejde o účetní tržbu ani potvrzení zaplacení." label="Odhad hodnoty návštěv za měsíc" value={formatCzkCents(dashboard.metrics.estimatedAttendedValueThisMonthCents)} /></div><div className="admin-analytics-grid"><DataCard title="Oblíbenost lekcí za 90 dní"><AnalyticsBars items={dashboard.classPopularity.map((item) => ({ label: item.className, value: item.reservations, detail: `${item.reservations} rezervací · ${item.attended} potvrzených návštěv` }))} zeroText="Zatím nejsou rezervace, ze kterých by šlo oblíbenost vyhodnotit." /></DataCard><DataCard title="Potvrzené návštěvy po týdnech"><AnalyticsBars items={dashboard.weeklyAttendance.map((item) => ({ label: formatWeek(item.weekStart), value: item.attended, detail: `${item.attended} návštěv` }))} zeroText="Docházka zatím není potvrzená. Označujte účast v sekci Rezervace." /></DataCard></div><DataCard title="Storna a neúčasti v tomto měsíci"><div className="admin-compact-metrics"><p><strong>{dashboard.metrics.lateCancellationsThisMonth}</strong><span>Pozdní storna</span></p><p><strong>{dashboard.metrics.noShowsThisMonth}</strong><span>Neúčasti</span></p></div><p className="admin-form-note">Finanční karta je záměrně označená jako odhad. Skutečnou tržbu lze zobrazit až po budoucím zavedení evidence zaplacení a permanentek.</p></DataCard><DataCard title="Dnešní lekce">{dashboard.today.length ? dashboard.today.map((item) => <SessionRow item={item} key={item.id} onCancel={cancelSession} busy={busy} />) : <Empty text="Dnes není naplánovaná žádná lekce." />}</DataCard></>}
    {tab === "schedule" && <><AdminHeading eyebrow="Provoz" title="Rozvrh a termíny" copy="Přidejte, upravte nebo bezpečně zrušte konkrétní termín a týdenní řadu." /><details className="admin-create" open><summary>Hromadná změna rozvrhu</summary><ScheduleBatchForm busy={busy} classTypes={classTypes} onSubmit={changeScheduleBatch} /></details><details className="admin-create"><summary>{editingSession ? `Upravit: ${editingSession.className}` : "Přidat termín"}</summary><SessionForm busy={busy} classTypes={classTypes} instructors={instructors} item={editingSession} onCancel={() => setEditingSession(null)} onSubmit={saveSession} /></details><DataCard title="Naplánované termíny">{sessions.length ? sessions.map((item) => <SessionRow item={item} key={item.id} onCancel={cancelSession} onEdit={setEditingSession} busy={busy} />) : <Empty text="Nejsou vytvořené žádné termíny." />}</DataCard></>}
    {tab === "classes" && <><AdminHeading eyebrow="Nabídka" title="Lekce a instruktoři" copy="Obsah, náročnost a fotografii upravíte zde; veřejný web se aktualizuje z jednoho zdroje." /><div className="admin-two-columns"><DataCard title={editingClass ? `Upravit: ${editingClass.name}` : "Typy lekcí"}><ClassTypeForm item={editingClass} busy={busy} nextSortOrder={classTypes.length * 10 + 10} onCancel={() => setEditingClass(null)} onSubmit={saveClass} /><ul className="admin-simple-list">{classTypes.map((item) => <li key={item.id}><div><strong>{item.name} · {item.difficulty}/5{item.active ? "" : " · skrytá"}</strong><span>{item.tagline}</span></div><button onClick={() => setEditingClass(item)} type="button">Upravit</button></li>)}</ul></DataCard><DataCard title="Instruktoři"><form className="admin-form" onSubmit={createInstructor}><Field label="Jméno" name="displayName" required /><Field label="Představení" name="bio" required /><button className="button button-small" disabled={busy} type="submit">Přidat instruktora</button></form><p className="admin-form-note">Odebraný instruktor zmizí z nabídky a z nových termínů. Starší lekce a jejich historie zůstanou zachované.</p><ul className="admin-simple-list">{instructors.filter((item) => item.active).map((item) => <li key={item.id}><div><strong>{item.displayName}</strong><span>{item.bio}</span><details><summary>Upravit profil a fotografii</summary><InstructorEditor person={item} classTypes={classTypes} onSaved={load} /></details></div><button disabled={busy} onClick={() => void toggleInstructor(item)} type="button">Odebrat</button></li>)}</ul></DataCard></div><DataCard title="Tým studia"><TeamEditor /></DataCard></>}
...
function Stat({ help, label, value }: { help?: string; label: string; value: number | string }) { return <article className="admin-stat"><span className="admin-stat-label">{label}{help && <HelpHint text={help} />}</span><strong>{value}</strong></article>; }
...
function AnalyticsBars({ items, zeroText }: { items: { detail: string; label: string; value: number }[]; zeroText: string }) {
  const maximum = Math.max(...items.map((item) => item.value), 0);
  if (!maximum) return <Empty text={zeroText} />;
  return <div className="admin-chart" role="list">{items.map((item) => <div aria-label={`${item.label}: ${item.detail}`} className="admin-chart-row" key={item.label} role="listitem"><div><strong>{item.label}</strong><span>{item.detail}</span></div><div aria-hidden="true" className="admin-chart-track"><span style={{ width: `${Math.max(item.value / maximum * 100, item.value ? 3 : 0)}%` }} /></div></div>)}</div>;
}
function formatWeek(value: string) { return `Týden od ${new Intl.DateTimeFormat("cs-CZ", { day: "numeric", month: "numeric" }).format(new Date(`${value}T12:00:00`))}`; }
function formatCzkCents(value: number) { return new Intl.NumberFormat("cs-CZ", { style: "currency", currency: "CZK", maximumFractionDigits: 0 }).format(value / 100); }
function ClassTypeForm({ busy, item, nextSortOrder, onCancel, onSubmit }: { busy: boolean; item: ClassType | null; nextSortOrder: number; onCancel: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => Promise<void> }) {
  ...
  return <form className="admin-form" onSubmit={(event) => void onSubmit(event)}>
    ...
    <TextArea defaultValue={value("benefits")} label="Hlavní přínosy" name="benefits" /><TextArea defaultValue={value("audience")} label="Pro koho je lekce" name="audience" /><TextArea defaultValue={value("defaultEquipment")} help="Pomůcky zajišťované studiem, například TRX, trampolína, balanční deska nebo podložka." label="Pomůcky ve studiu" name="defaultEquipment" /><TextArea defaultValue={value("whatToBring", "Sportovní oblečení, pohodlnou obuv a pití.")} help="Osobní věci klientky. Podložky ani cvičební pomůcky sem neuvádějte, protože je připravuje studio." label="Co si vzít" name="whatToBring" /><TextArea defaultValue={value("practicalNotice")} label="Praktické upozornění" name="practicalNotice" />
...
function SessionForm({ busy, classTypes, instructors, item, onCancel, onSubmit }: { busy: boolean; classTypes: ClassType[]; instructors: Instructor[]; item: Session | null; onCancel: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => Promise<void> }) {
  const duration = item ? Math.round((new Date(item.endAt).getTime() - new Date(item.startAt).getTime()) / 60_000) : 60;
  const availableTypes = classTypes.filter((type) => type.active || type.id === item?.classTypeId);
  const availableInstructors = instructors.filter((instructor) => instructor.active || instructor.id === item?.instructorId);
  return <form className="admin-form admin-form-grid" key={item?.id ?? "new-session"} onSubmit={(event) => void onSubmit(event)}>
    <Select {...(item ? { defaultValue: item.classTypeId } : {})} help="Skryté typy nelze vybrat pro nový termín; u staršího termínu zůstávají viditelné pro bezpečnou opravu." label="Typ lekce" name="classTypeId" options={availableTypes.map((type) => ({ label: type.name, value: type.id }))} />
    <Select {...(item ? { defaultValue: item.instructorId } : {})} help="Konkrétní termín se vždy řídí zde vybraným aktivním instruktorem." label="Instruktor" name="instructorId" options={availableInstructors.map((instructor) => ({ label: instructor.displayName, value: instructor.id }))} />
    <Field {...(item ? { defaultValue: studioDateTimeInput(item.startAt) } : {})} help="Místní datum a čas ve studiu (Europe/Prague)." label="Začátek" name="startAt" type="datetime-local" required />
    <Field defaultValue={String(duration)} label="Délka (min)" name="durationMinutes" type="number" required />
    <Field defaultValue={String(item?.capacity ?? 10)} help="Kapacitu nelze snížit pod počet aktivních rezervací. Veřejný web počet míst nezobrazuje." label="Kapacita" name="capacity" type="number" required />
    <Field defaultValue={String((item?.priceCents ?? 25_000) / 100)} help="Cena jedné návštěvy placená ve studiu." label="Cena (Kč)" name="price" type="number" required />
    <Field defaultValue={String(item?.arrivalLeadMinutes ?? 10)} label="Doporučený příchod (min)" name="arrivalLeadMinutes" type="number" required />
    <Field defaultValue={item?.locationName ?? "Studio Balance"} label="Místo" name="locationName" required />
    <Field defaultValue={item?.locationAddress ?? "Ruská 10, 792 01 Bruntál"} label="Adresa" name="locationAddress" required />
    <Field defaultValue={item?.equipment ?? "Všechny pomůcky potřebné pro lekci jsou připravené ve studiu."} help="Vybavení, které připraví studio." label="Pomůcky ve studiu" name="equipment" required />
    <Field defaultValue={item?.suitability ?? "Lekci lze přizpůsobit začátečnicím i pokročilým."} label="Vhodnost" name="suitability" required />
    {item && <TextArea defaultValue={item.changeNotice ?? ""} help="Stručně vysvětlete změnu klientkám. Text se uloží u termínu a použije v provozním oznámení." label="Důvod změny" name="changeReason" required />}
    <div className="admin-row-actions"><button className="button" disabled={busy} type="submit">{item ? "Uložit změnu termínu" : "Přidat termín"}</button>{item && <button onClick={onCancel} type="button">Zrušit úpravy</button>}</div>
  </form>;
}
function SessionRow({ item, onCancel, onEdit, busy }: { busy: boolean; item: Session; onCancel: (item: Session) => Promise<void>; onEdit?: (item: Session) => void }) { return <article className="admin-session-row"><time>{formatStudioDate(item.startAt,{weekday:"short",day:"numeric",month:"numeric",hour:"2-digit",minute:"2-digit"})}</time><div><strong>{item.className}</strong><span>{item.instructorName} · {item.locationName}</span></div><span>{item.bookingCount} / {item.capacity} rezervací</span><span>{item.status === "scheduled" ? "Naplánováno" : "Zrušeno"}</span>{item.status === "scheduled" && <div className="admin-row-actions">{onEdit && <button disabled={busy} onClick={() => onEdit(item)} type="button">Upravit</button>}<button disabled={busy} onClick={() => void onCancel(item)} type="button">Zrušit</button></div>}</article>; }
function studioDateTimeInput(value: string) {
  const parts = Object.fromEntries(new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Prague", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).formatToParts(new Date(value)).map((part) => [part.type, part.value]));
...
function Select({ defaultValue, help, label, name, options }: { defaultValue?: string; help?: string; label: string; name: string; options: { label: string; value: string }[] }) { return <label><Label help={help} text={label} /><select defaultValue={defaultValue} name={name} required>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>; }
