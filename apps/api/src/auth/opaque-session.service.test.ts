...
const identity = { subject: "subject-1", email: "client@example.test", emailVerified: false, roles: ["client"] as ("client" | "admin" | "super_admin")[] };
...
describe("OpaqueSessionService", () => {
  it("persists only a hash in place of the browser token and encrypts the refresh token", async () => {
    ...
    expect(parameters[8]).not.toBe("refresh-token-secret-value");
    expect(parameters[8]).toMatch(/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/);
  });
  ...
  it("revalidates a stale role snapshot before returning a session", async () => {
    ...
    row.refresh_token_ciphertext = database.query.mock.calls[0]?.[1]?.[8] as unknown as string;
    vi.spyOn(service as unknown as { refresh: () => Promise<unknown> }, "refresh").mockResolvedValue({ subject: identity.subject, session: { ...identity, roles: ["client", "admin"] }, refreshToken: "rotated-refresh-token" });
